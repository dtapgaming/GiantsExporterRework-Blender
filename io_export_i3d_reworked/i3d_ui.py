...
class I3D_OT_ShowDeltaBakeLocation(bpy.types.Operator):
    ...
    bl_description = "Highlights the real Delta Bake button in the Data tab"

    def execute(self, context):

        # Find a Properties Editor already open
        props_area = None
        ...
        # Switch that Properties Editor to DATA tab
        ...
        # Start flashing
        global _flash_counter, _flash_state
        _flash_counter = FLASH_DURATION * 2    # 4 red flashes
        _flash_state = True
...
g_loadedMaterialTemplates = {
    "templates": {}
}

g_selectedMaterialTemplateCategory = None

def get_game_install_path_from_prefs():
    """Return the game installation directory stored in addon preferences, or ''."""
    ...
    if addon_entry and getattr(addon_entry, "preferences", None):
        return addon_entry.preferences.game_install_path or ""
    return ""

class I3D_OT_pick_game_folder(bpy.types.Operator):
    bl_idname = "i3d.pick_game_folder"
    ...
    def execute(self, context):
        addon_entry = bpy.context.preferences.addons.get("io_export_i3d_reworked")
        if addon_entry and addon_entry.preferences:
            addon_entry.preferences.game_install_path = self.directory

        # also update scene display field (harmless, used only as fallback now)
        context.scene.I3D_UIexportSettings.i3D_gameLocationDisplay = self.directory
...
def extractXMLShaderData():
    # Use the shader folder location; if it contains '$' substitute the game
    # installation directory stored in addon preferences.
    game_path = get_game_install_path_from_prefs()
    dirPath = bpy.context.scene.I3D_UIexportSettings.i3D_shaderFolderLocation.replace("$", game_path)
...
def updateShaderFolderLocation(context):
    ...
    it is set in addon preferences, or to an absolute path otherwise. """
    ...
    if game_path:
        # Store as "$" + relative path from the game install directory
        path = "$" + os.path.relpath(path, start=game_path)
    else:
        # Fall back to a plain absolute path
...
# -------------------------------------------
        ...
        if   'exp'  == context.scene.I3D_UIexportSettings.UI_settingsMode:
            ...
            # -----------------------------------------
            # "Game Location" box removed to allow for memory of game location per user so user doesnt need to set it every time now set in user preferences in the addons specific prefferences
            # --------------------------
            ...
            # -----------------------------------------
            # Output File box (ALWAYS visible - NOT inside missing_path)
            # -----------------------------------------
            box = layout.box()
            row = box.row()
            row.prop(
            ...
            if context.scene.I3D_UIexportSettings.i3D_exportUseSoftwareFileName:
                ...
                row.enabled = not (context.scene.I3D_UIexportSettings.i3D_exportFileLocation == "")
        ...
        #-----------------------------------------
            ...
            if context.scene.I3D_UIexportSettings.UI_customTools:
                ...
                try:
                    ...
                    pass
                #-----------------------------------------
                # Delta Vertex Color Tool
                #-----------------------------------------
                delta_box = col.box()
                row = delta_box.row()
                row.label(text="Delta → Vertex Color (Roof Snow Heap Tool)", icon="VPAINT_HLT")

                # --- Row with YouTube buttons ---
                row = delta_box.row()
                ...
                #------------------------------------------------------------
                # Clean, safe, separate block for toggle + flash button
                #------------------------------------------------------------
                outer_box = col.box()
...
class I3D_OT_PanelAddShader_ButtonAdd( bpy.types.Operator):
    ...
    def execute(self,context):
        ...
        if materialObj:
            ...
            dirPath = dirPath.replace("$", getGamePath())
            ...
            if context.scene.I3D_UIexportSettings.i3D_shaderFolderLocation[0] == "$":
                ...
                # FIX: Don't convert shader paths to external-relative paths
                if context.scene.I3D_UIexportSettings.i3D_exportRelativePaths:
                    # external-relative requested BUT shaders should NOT be external-relative
...
class I3D_OT_PanelOpenFolderFilebrowser(bpy.types.Operator,bpy_extras.io_utils.ImportHelper):
    ...
    def execute(self, context):
        ...
        if self.state == 1:  # Game location
            ...
            # update the scene setting (what the UI uses)
            context.scene.I3D_UIexportSettings.i3D_gameLocationDisplay = abspath

            # also try to save into addon preferences, if available
            addon_entry = bpy.context.preferences.addons.get("io_export_i3d_reworked")
            if addon_entry is not None:
                addon_entry.preferences.game_install_path = abspath

        elif self.state == 2:  # Shaders
            # KEEP your existing shader path code here as-is
            path = filename.rsplit("\\",1)[0] + "\\"    #remove filename and extension
            context.scene.I3D_UIexportSettings.i3D_shaderFolderLocation = path
            updateShaderFolderLocation(context)
            if not context.scene.I3D_UIexportSettings.i3D_shaderEnum in [t[0] for t in I3D_PT_PanelExport.getShadersFromDirectory(self,context)]:
                context.scene.I3D_UIexportSettings.i3D_shaderEnum = I3D_PT_PanelExport.getShadersFromDirectory(self,context)[0][0]
...
class I3D_OT_PanelSetGameShader(bpy.types.Operator):
    """ Automatically find the shader folder inside the Game Installation """
    ...
    def execute(self, context):
        # 1) Try addon preferences first (new behavior)
        gameInstallationPath = get_game_install_path_from_prefs()

        # 2) If prefs are empty, fall back to the old scene field (for backward compatibility)
        if not gameInstallationPath:
            addon_entry = bpy.context.preferences.addons.get("io_export_i3d_reworked")
            if addon_entry and addon_entry.preferences:
                gameInstallationPath = addon_entry.preferences.game_install_path
            else:
                gameInstallationPath = ""


        # 3) If still empty, try auto-detect (original behavior)
        if not gameInstallationPath:
        ...
        # 5) Store shader folder location (still using the game path as base)
...
class I3D_PT_DeltaVertex_ToolsPanel(bpy.types.Panel):
    bl_label = "DELTA TO VERTEX COLOR"
    bl_idname = "I3D_PT_delta_vertex_tools"
    bl_description = "Watch this Video for how to"
    bl_space_type = "VIEW_3D"
    bl_region_type = "UI"
    bl_category = "GIANTS I3D Exporter REWORKED"
    bl_parent_id = "I3D_PT_PanelTools"
    ...
    def draw(self, context):
        ...
        row = col.row()
        row.alignment = 'CENTER'
        row.operator("delta_to_vertexcolor.bake", text="Bake Deltas to Vertex Colors")
    ...
    def execute(self, context):
        ...
        if self.state == 0:
            try:
                ...
                # Build the full path to the templates XML using the configured game path
                materialTemplateFilename = "$data/shared/detailLibrary/materialTemplates.xml"
                game_path = getGamePath() or ""

                if not game_path:
                    print("[MaterialTemplates] ERROR: FS25 Game Path is empty in addon preferences – cannot load materialTemplates.xml.")
                    return {'CANCELLED'}

                # Ensure game_path ends with a separator
                if not game_path.endswith(("\\", "/")):
                    game_path = game_path + os.sep

                templatesXmlFilename = materialTemplateFilename.replace("$", game_path)
                templatesXmlFilename = os.path.normpath(templatesXmlFilename)

                if not os.path.isfile(templatesXmlFilename):
                    print(f"[MaterialTemplates] ERROR: Material templates file not found: {templatesXmlFilename}")
                    return {'CANCELLED'}

                try:
                    xmlTree = xml_ET.parse(templatesXmlFilename)
                except xml_ET.ParseError as err:
                    msg = "Failed to load parameter templates from '%s': %s" % (templatesXmlFilename, err)
                    print("[MaterialTemplates] ERROR:", msg)
                    return {'CANCELLED'}

                # If we got here, the XML loaded fine – original logic follows
                ...
                for template in templatesFileRoot.findall("template"):
                    ...
                    iconFilenamePath = iconFilename.replace("$", getGamePath())
                    ...
                    for category in categories:
                        ...
                        if category not in categoryDict:
                            ...
                            categoryDict[category]["thumbnails"] = previews.new()
                ...
                msg = f"I3D_OT_PanelMaterial_OpenMaterialTemplatesWindowButton::execute::cancelled1 ({type(e).__name__}: {e})"
                print("[MaterialTemplates] ERROR in execute (state 0):", msg)
                return {'CANCELLED'}
            ...
            try:
                ...
                msg = f"I3D_OT_PanelMaterial_OpenMaterialTemplatesWindowButton::execute::cancelled2 ({type(e).__name__}: {e})"
                print("[MaterialTemplates] ERROR in execute (state 1):", msg)
                return {'CANCELLED'}
...
class I3D_UIexportSettings( bpy.types.PropertyGroup ):
    ...
    UI_gameLocation         : bpy.props.BoolProperty   ( name = "Game Location",         default = True )
    ...
    i3D_exportRelativePaths       : bpy.props.BoolProperty   ( name = "(no longer Used)",description="Export File Paths relative to the *.i3d File",      default = dcc.SETTINGS_UI['i3D_exportRelativePaths']['defaultValue'])
    ...
    i3D_exportSnowHeapMode         : bpy.props.BoolProperty ( name = "SnowHeap Mode", description = "Only enable this if you are making a snow heap roof mask to use with the snowHeapShader in FS25.", default = False )
    i3D_exportGameRelativePath       : bpy.props.BoolProperty   ( name = "(Future Feature)",description="Export File Paths relative to the Game Installation Path",       default = dcc.SETTINGS_UI['i3D_exportGameRelativePath']['defaultValue'], update=setExportRelativePath  )
...
def register():
    ...
    bpy.utils.register_class( I3D_OT_ShowDeltaBakeLocation )
