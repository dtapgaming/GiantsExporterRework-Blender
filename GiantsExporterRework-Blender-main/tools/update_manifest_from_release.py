...
def parse_build(tag: str) -> int:
    # Build number is optional and can be encoded in several tag styles:
    #   - 10.0.18.2
    #   - 10.0.18-alpha.2 / 10.0.18-ALPHA2 / 10.0.18.ALPHA2
    #   - 10.0.18BETA3 / 10.0.18.BETA3
    #
    # If no build is present, return 0.
    if not tag:
        return 0

    # 1) Explicit 4th numeric component: 10.0.18.2
    m = re.search(r"\b\d+\.\d+\.\d+\.(\d+)\b", tag)
    if m:
        return int(m.group(1))

    # 2) Suffix after channel marker: ALPHA2 / BETA3 / RC4 / BUILD12
    m = re.search(r"\b(?:alpha|beta|rc|build)[\-\._ ]?(\d+)\b", tag, re.IGNORECASE)
    if m:
        return int(m.group(1))
